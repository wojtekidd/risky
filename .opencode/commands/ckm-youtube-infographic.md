---
description: "Convert YouTube video to visual infographic - Args: <youtube-url> [style]"
---
...
1. **Extract Video Data**
   ...
   - Run VidCap API:
     ```bash
     python .opencode/skills/youtube-handling/scripts/vidcap.py info "<url>"
     python .opencode/skills/youtube-handling/scripts/vidcap.py summary "<url>" --screenshot
     python .opencode/skills/youtube-handling/scripts/vidcap.py caption "<url>" --ext vtt
     ```
...
3. **Design Infographic**
   ...
   - Inject brand context:
     ```bash
     node .opencode/skills/brand-guidelines/scripts/inject-brand-context.cjs --json
     ```
...
<head>
  ...
  <style>
    ...
    :root {
      /* Paste output from: node .opencode/skills/design-system/scripts/embed-tokens.cjs --minimal */
    }
  </style>
  <style>
    body {
