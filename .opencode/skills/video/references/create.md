...
Reference: `.opencode/skills/video/references/audio-directive-guide.md`
...
```bash
node .opencode/skills/video/scripts/analyze-video.cjs \
...
```bash
node .opencode/skills/video/scripts/optimize-for-platform.cjs \
...
```bash
node .opencode/skills/video/scripts/extract-captions.cjs \
  --video "assets/videos/{date}-{slug}/master.mp4" \
  --format srt \
