...
Keep `.opencode/` directory in version control:
...
Copy `.opencode/.env.example` to `.opencode/.env` and fill in:
- GITHUB_TOKEN
- DATABASE_URL
```
...
- `.opencode/skills/`
- `.opencode/hooks.json`
- `.opencode/mcp.json` (without secrets)
- `.opencode/.env.example`

**What NOT to commit:**
- `.opencode/.env` (contains secrets)
- `.opencode/memory/` (optional)
- `.opencode/cache/`
- API keys or tokens
...
echo 'ANTHROPIC_API_KEY=sk-ant-xxxxx' > .opencode/.env
```
...
```bash
# Check hooks.json
cat .opencode/hooks.json | jq .

# Review scripts
cat .opencode/scripts/hook.sh
...
```bash
#!/bin/bash
# .opencode/scripts/rate-limit.sh

REQUESTS_FILE=".opencode/requests.log"
MAX_REQUESTS=100
WINDOW=3600  # 1 hour
...
```bash
# Create team plugin
cd .opencode/plugins/team-plugin
cat > plugin.json <<EOF
{
  "name": "team-plugin",
  "skills": ["skills/*/"],
...
```bash
#!/bin/bash
# .opencode/scripts/budget-check.sh
