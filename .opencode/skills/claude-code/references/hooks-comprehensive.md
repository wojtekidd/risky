...
**Configuration**: `.opencode/settings.json` (4-tier hierarchy)
...
2. **User settings**: `~/.opencode/settings.json` (global defaults)
3. **Project settings**: `.opencode/settings.json` (team-shared, version controlled)
4. **Local settings**: `.opencode/settings.local.json` (personal, git-ignored)
...
{
  "type": "command",
  "command": "jq -r '.tool_parameters.command' | tee -a .opencode/commands.log"
}
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_parameters | \"\\(.command) - \\(.description)\"' >> .opencode/commands.log"
          }
        ]
      }
    ]
...
Edit `.opencode/settings.json` or `.opencode/settings.local.json`:
...
```bash
#!/bin/bash
CACHE_FILE=".opencode/cache/format-cache"
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .opencode/scripts/validate-bash.sh"
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .opencode/scripts/protect-files.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .opencode/scripts/auto-format.sh"
          }
        ]
      }
    ],
    "Notification": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .opencode/scripts/desktop-notify.sh"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .opencode/scripts/log-usage.sh"
          }
        ]
      }
    ],
