...
3. **Project settings** (`.opencode/settings.json`)
4. **Global settings** (`~/.opencode/settings.json`)
...
```json
{
  "logging": {
    "level": "info",
    "file": ".opencode/logs/session.log",
    "console": true
  }
}
```
...
Create `~/.opencode/output-styles/my-style.md`:
...
### Project Settings
- Keep project-specific in `.opencode/settings.json`
