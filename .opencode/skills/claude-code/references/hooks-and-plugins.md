...
Hooks are configured in `.opencode/hooks.json`:
...
```bash
#!/bin/bash
# .opencode/scripts/validate-bash.sh
...
```json
{
  "hooks": {
    "pre-tool": {
      "bash": "./.opencode/scripts/validate-bash.sh"
    }
  }
}
```
...
```bash
#!/bin/bash
# .opencode/scripts/format-code.sh
...
```json
{
  "hooks": {
    "post-tool": {
      "write": "./.opencode/scripts/format-code.sh",
      "edit": "./.opencode/scripts/format-code.sh"
    }
  }
}
```
...
# Log prompt
echo "$(date): $USER_PROMPT" >> .opencode/usage.log
...
{
  "hooks": {
    "user-prompt-submit": "./.opencode/scripts/track-usage.sh"
  }
}
```
