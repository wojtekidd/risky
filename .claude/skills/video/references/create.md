...
Reference: `.claude/skills/video/references/audio-directive-guide.md`
...
```bash
node .claude/skills/video/scripts/analyze-video.cjs \
...
```bash
node .claude/skills/video/scripts/optimize-for-platform.cjs \
...
```bash
node .claude/skills/video/scripts/extract-captions.cjs \
  --video "assets/videos/{date}-{slug}/master.mp4" \
  --format srt \
